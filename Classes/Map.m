...
methods 
        ...
        if is_closed == false
            for i = 1:length(observation_vector)
                ...
                for j = 1:length(landmarks)
                    ...
                    if d_obs_land < map.conf.malhanobis_for_correspondence
                        index = find(P2==j);
                        if length(index) > 0
    ...
    % I can compute the centroids of the new observations and compare it with the old ones
    % If I find an high correspondence, then a loop is detected
    function [P1,P2,is_closed] = loop_closure(map, observation_vector, robot,k_iter)
        P1 = [];
        P2 = [];
        is_closed = false;
        landmarks = map.landmark_vector;
        ...
        if length(observation_vector) > map.conf.min_number_observations_closure && length(land_inside) > floor(length(observation_vector)*map.conf.ratio_features_observation_closure)
        ...
        theta_test = linspace(-map.conf.max_testing_angle_closure,map.conf.max_testing_angle_closure,map.conf.N_theta_closure).*pi/180;
                ...
                for j = 1:length(theta_test)
                    ...
                    for i = 1:length(absolute_obs_vector) 
                        ...
                        for k = 1:length(land_inside)
                            d_obs_land = mahalanobis_distance([trxx(i),tryy(i)]', landmarks(land_inside(k)).x, landmarks(land_inside(k)).P);
                            if d_obs_land < map.conf.malhanobis_for_closure
    ...
    function [trxx,tryy] = rototrasl(map,delta_centroids,abs_observation,t,centroid_land)
        R = [cos(t), -sin(t); sin(t), cos(t)];

        traslation = abs_observation.x + delta_centroids;

        TR = centroid_land + R*(traslation-centroid_land);
        trxx = TR(1);
        tryy = TR(2);
    ...
    function land_inside = old_landmark_inside_rectangle(map,robot,observation_vector,radiuspar,radiusper)
        ...
        element_deleted = map.conf.ratio_map_observation_closure*length(observation_vector);
        ...
        for i = 1:map.size() - element_deleted
    ...
    function nw_land = up_map(map,robot, observation_new)
        ...
        if length(observation_new) > 0
            buffer_max = map.conf.buffer_length;
            ...
            if length(map.landmark_buffer) > 1
                for i = 1:size(map.landmark_buffer{1},2)
                    ...
                    if minn < map.conf.malhanobis_for_update_map % treshold of distance
            ...
            for i = 1:map.size()
                for j = 1:length(nw_land)
                    if mahalanobis_distance(map.landmark_vector(i).x,landmark_observed1(j).x,map.landmark_vector(i).P) < map.conf.malhanobis_for_controlling_new_features 
                        tmp3(j) = nw_land(j);
                    end
                end
            end
